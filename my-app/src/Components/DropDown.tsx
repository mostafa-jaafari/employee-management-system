...
export function DropDown({ Label, Options, selectedLabel, HandleSelectOption, className, DefaultAllButton = true }: { Label: string; Options: string[]; selectedLabel: string; HandleSelectOption: (option: string, e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => void; className: string; DefaultAllButton?: boolean; }) {
    ...
    return (
        ...
        >
            <h2 className={`font-[500] text-nowrap ${selectedLabel === "" ? "capitalize" : "lowercase"} w-full ${isOpen ? "text-white" : "text-neutral-300"}`}>{selectedLabel !== "" ? selectedLabel : Label ? Label : "Label"}</h2> <FaChevronDown size={12} className={`${isOpen ? "rotate-180 text-white" : "text-neutral-300"} transition-transform duration-200`}/>
            {isOpen && (
                <motion.div
                    key="dropdown"
                        ...
                        {DefaultAllButton && (
                            <button
                                ...
                                className="px-1.5 py-1 text-neutral-400 hover:text-neutral-200 
                                    ...
                                    text-start border-b border-neutral-700/60"
                                >
                                All
                            </button>
                        )}
                        {Options.map((opt, idx) => {
                            return (
                                <button
                                    ...
                                    className={`px-1.5 py-1 text-neutral-400 hover:text-neutral-200 hover:bg-neutral-700/60 cursor-pointer text-start
